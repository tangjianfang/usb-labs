...
格式: [%(asctime)s][%(levelname)s][%(name)s] %(message)s —— 与 C++
`[时间][级别][模块][线程] 消息` 的前三段一致（Python logging 无线程段，规格允许）。
级别语义对齐 C++: trace=逐包/逐字节级 debug=逐操作 info=生命周期 warn=可恢复
err=失败 off=静默。级别开关 USBTS_LOG_LEVEL 与 C++ 侧同名同值域
...
def setup(name, level=None, logfile=None):
    ...
    lg.propagate = False          # 模块各自成流，防父 logger（如 "usbtest"）二次emit
